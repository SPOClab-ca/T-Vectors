...
if __name__ == '__main__':
    ...
    parser.add_argument('--sample-crop', default=0, type=int, help="How many samples to sacrifice for crop and resample"
                                                                   " augmentation.")
    parser.add_argument('--batch-crop-max', default=0.8, type=float, help='The maxmimum fraction of temporal cropping '
                                                                          'for retrieved batches.')
    parser.add_argument("--log-interval", default=1000, type=int, help="How many batches between logging output.")
    ...
    if args.resume is not None:
        t_vectors.load('checkpoints/t-vectors.pt')
    ...
    def knn_validation(metrics):
        if args.knn_val > 0:
            ...
            x, extras = create_numpy_formatted_ds(validating, t_vectors)
            ...
            for train, test in model_selection.StratifiedKFold(n_splits=5).split(x, y):
                ...
                tqdm.tqdm.write("{:.1%}".format(model.score(x[test, :], y[test].squeeze())))
    ...
    process.fit(training, epochs=experiment.training_params.epochs, batch_size=experiment.training_params.batch_size,
                ...
                resume_epoch=args.resume, sampler=sampler, log_callback=knn_validation)
